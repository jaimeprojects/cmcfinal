...
//    }
 ...
 {
	 int add =dblib.user_addUser(ac.getFirstName(), ac.getLastName(), ac.getUsername(), ac.getPassword(), ac.getType());
	 if(add == 1)
	 {
		 return false;
	 }
  ...
  */
  ...
  public ArrayList<String> getRecommendedSchools(University u)
  {
...
public void deleteUser(String username)
{
	...
	for(int i = 0; i<savedSchools.length; i++)
	{
		...
		{
			for (int j = 0; j<savedSchools[i].length; j++)
			{
				dblib.user_removeSchool(username, savedSchools[i][j]);
			}
		}
...
public void deleteSavedSchool(String username, String universityName)
  {
	dblib.user_removeSchool(username, universityName);
  }
}
